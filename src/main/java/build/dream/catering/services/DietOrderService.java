...
public class DietOrderService {
    ...
    public ApiRest saveDietOrder(SaveDietOrderModel saveDietOrderModel) {
        ...
        Map<String, List<DietOrderDetailGoodsFlavor>> dietOrderDetailGoodsFlavorMap = new HashMap<String, List<DietOrderDetailGoodsFlavor>>();
        ...
        if (packageFee.compareTo(BigDecimal.ZERO) > 0) {
            ...
            DietOrderDetail dietOrderDetail = DietOrderDetail.builder().tenantId(tenantId).tenantCode(tenantCode).branchId(branchId).dietOrderId(dietOrderId).dietOrderGroupId(extraDietOrderGroup.getId()).goodsType(4).goodsId(Constants.BIG_INTEGER_MINUS_TWO).goodsName("打包费").goodsSpecificationId(Constants.BIG_INTEGER_MINUS_TWO).goodsSpecificationName("").categoryId(Constants.BIG_INTEGER_MINUS_ONE).categoryName("打包费").price(packageFee).flavorIncrease(BigDecimal.ZERO).quantity(BigDecimal.ONE).totalAmount(packageFee).discountAmount(BigDecimal.ZERO).payableAmount(packageFee).paidAmount(BigDecimal.ZERO).createUserId(userId).lastUpdateUserId(userId).build();
            dietOrderDetails.add(dietOrderDetail);
        }

        DatabaseHelper.insertAll(dietOrderDetails);

        if (MapUtils.isNotEmpty(dietOrderDetailGoodsFlavorMap)) {
            List<DietOrderDetailGoodsFlavor> dietOrderDetailGoodsFlavors = new ArrayList<DietOrderDetailGoodsFlavor>();
            for (Map.Entry<String, List<DietOrderDetailGoodsFlavor>> entry : dietOrderDetailGoodsFlavorMap.entrySet()) {
                String key = entry.getKey();
                DietOrderDetail dietOrderDetail = dietOrderDetailMap.get(key);
                BigInteger dietOrderDetailId = dietOrderDetail.getId();

                List<DietOrderDetailGoodsFlavor> value = entry.getValue();
                for (DietOrderDetailGoodsFlavor dietOrderDetailGoodsFlavor : value) {
